...
import com.calistropo.models.{CountPerTile, FilledTiles, VehicleEvent, VehicleRawEvent}
import com.calistropo.pipes._
import com.calistropo.util.Idioms._
...
object Main {
  ...
  def main(args: Array[String]) = {
    ...
    args.head match {
      ...
      case name@"save_vehicle_events" =>
        ...
        PullFromKafka[VehicleEvent](KafkaConfig(streamingContext = ssc,
          ...
          groupId = name: String,
          bootstrapServers = Set("localhost:9092"): Set[String])
        ) |> Log2Know(name) |> Send2Cassandra[VehicleEvent]("calis", "vehicle_events")
      ...
      case name@"save_counts_per_tile" =>
        ...
        PullFromKafka[CountPerTile](KafkaConfig(streamingContext = ssc,
          ...
          groupId = name: String,
          bootstrapServers = Set("localhost:9092"): Set[String])
        ) |> Log2Know(name) |> Send2Cassandra[CountPerTile]("calis", "counts_per_tile")
      case _ =>
    ...
    //sbt package && ~/tools/spark-2.2.0-bin-hadoop2.7/bin/spark-submit --class "com.calistropo.Main" --packages com.typesafe.play:play-json_2.11:2.6.6,org.apache.spark:spark-streaming-kafka-0-10_2.11:2.2.0 --master local[4] target/scala-2.11/hopkins_2.11-0.1.0-SNAPSHOT.jar 1
